...
public class MerchantStatusAdapter extends RecyclerView.Adapter<MerchantStatusAdapter.MerchantStatusViewHolder> {
    ...
    @Override
    public void onBindViewHolder(MerchantStatusViewHolder merchantStatusViewHolder, int position) {
        MerchantStatusModel merchantStatusModel = merchantStatusList.get(position);

        merchantStatusViewHolder.txtMerchantTitle.setText(merchantStatusModel.getMerchantTitle());
        merchantStatusViewHolder.txtMerchantAddress.setText(merchantStatusModel.getMerchantAddress());
        merchantStatusViewHolder.txtMerchantShortCode.setText(merchantStatusModel.getMerchantShortCode());
        if (merchantStatusModel.getStatus().equals(ACTIVE_STATUS)) {
            merchantStatusViewHolder.view.setBackgroundColor(context.getResources().getColor(R.color.green));
        }else if (merchantStatusModel.getStatus().equals(PENDING_STATUS)) {
            merchantStatusViewHolder.view.setBackgroundColor(context.getResources().getColor(R.color.yellow_dark));
        } else if (merchantStatusModel.getStatus().equals(DRAFTED_STATUS)){
            merchantStatusViewHolder.view.setBackgroundColor(context.getResources().getColor(R.color.red_selected));
        }
    }
